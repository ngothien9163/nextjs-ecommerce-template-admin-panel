...
interface BlogPostImageSelectorProps {
  value?: string;
...
export const BlogPostImageSelector: React.FC<BlogPostImageSelectorProps> = ({
  value,
  onChange,
  ...
  const handleSelect = (media: Media) => {
    console.log('📝 [BlogPostImageSelector] SAVE - Selected image ID:', media.id);
    setSelectedMedia(media);
    if (onChange) {
      console.log('📝 [BlogPostImageSelector] SAVE - Setting form field to:', media.id);
      onChange(media.id);
    }
    if (onSelect) {
      onSelect(media);
      ...
      >
        <div style={{ textAlign: 'center' }}>
          <Image
            ...
            fallback="data:image/png;base64,"
          />
        </div>
      </Modal>
    </div>
